$(document).ready(function () {
	RTA.storage.addInitializedListener(() => {
		var tabCounter = 1;
		...
		$(function () {
			...
			function addTab(name, client, oldload) {
				...
				if (oldload !== true) {
					for (var x in servers) {
						if (servers[x].name == name || name === null || name === "") {
							alert(
								"This name is already in use by another server, or invalid."
							);
							return false;
						}
					}
				...
				var tabContentHtml = RTA.clients.config.getConfig(
					client || tabClient.val(),
					name
				);
			...
			for (var i in servers) {
				...
				var mySettingInputs = $("#servertabs-" + (parseInt(i) + 1))
					.find("input, select")
					.get();
						...
						switch (mySettingInput.name) {
							case "dirlist":
								...
								thisTd.find("button[name=deldirbutton]").click(function () {
									$(this)
										.parents("td")
										.find("select[name=dirlist] option:selected")
										.remove();
									saveServersSettings();
								});
								break;
							case "labellist":
								...
								thisTd.find("button[name=dellabelbutton]").click(function () {
									$(this)
										.parents("td")
										.find("select[name=labellist] option:selected")
										.remove();
									saveServersSettings();
								});
								break;
							case "autolabellist":
								var thisTd = $(mySettingInput).parents("td");
								thisTd
									.find("button[name=addautolabelbutton]")
									.click(function () {
										var answer = prompt(
											"Enter a new tracker url / label combination like this:\nSOMETRACKER.COM,SOMELABEL"
										);
										if (answer !== null) {
											$(this)
												.parents("td")
												.find("select[name=autolabellist]")
												.append(
													$("<option>", {
														text: answer,
													})
								...
								thisTd
									.find("button[name=delautolabelbutton]")
									.click(function () {
										$(this)
											.parents("td")
											.find("select[name=autolabellist] option:selected")
											.remove();
										saveServersSettings();
									});
								break;
							case "autodirlist":
								...
								thisTd.find("button[name=addautodirbutton]").click(function () {
									var answer = prompt(
										"Enter a new tracker url / directory combination like this:\nSOMETRACKER.COM,SOMEDIRECTORY"
									);
									if (answer !== null) {
										$(this)
											.parents("td")
											.find("select[name=autodirlist]")
											.append(
												$("<option>", {
													text: answer,
												})
								...
								thisTd.find("button[name=delautodirbutton]").click(function () {
									$(this)
										.parents("td")
										.find("select[name=autodirlist] option:selected")
										.remove();
									saveServersSettings();
								});
								break;
...
function loadGeneralSettings() {
	var e = document.querySelectorAll(
		"#linksfoundindicator,#showpopups,#popupduration,#hearpopups,#catchfromcontextmenu,#catchfrompage,#linkmatches,#catchfromnewtab,#registerDelay"
	);
	for (key in e) {
		getSetting(e[key]);
	}
...
function flipVisibility(checkname, changename) {
	document.getElementById(changename).disabled = document.getElementById(
		checkname
	).checked
		? false
		: true;
...
function addMatch() {
	var newMatch = prompt(
		"Enter a partial string of a link that should be caught by the extension",
		""
	);
	if (!newMatch) return;
			...
			reader.onload = function () {
				...
				try {
					...
					resultField.innerHTML =
						"Result: &#x274C; Couldn't parse the input file.";
				}
			};
		...
		for (var u in elements) {
			...
			switch (element.type) {
				case "checkbox":
					...
					break;
				case "select-multiple":
					server[element.name] = JSON.stringify(
						Array.prototype.map.call(element.options, function (x) {
							return x.value;
						})
					);
					break;
